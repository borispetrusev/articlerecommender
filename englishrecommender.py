from hackernews import Thehackernews
...
class English_recommender:
    
    def __init__(self, user_name, PATH):
        self.user_name = user_name
        self.path = PATH
        self.training_dataset = pd.read_csv(PATH+'english_training_'+self.user_name+'.csv')
        self.training_dataset = self.training_dataset[~self.training_dataset['lemmatize_title'].isna()]
        self.training_dataset = self.training_dataset.dropna()
        try:
    ...
    def get_all_data(self):
        techcrunch = Techcrunch(self.user_name, self.path)
        techcrunch.retrieve_data()
        techcrunch.lemmatize_data()
        thehackernews = Thehackernews(self.user_name, self.path)
        thehackernews.retrieve_data()
        thehackernews.lemmatize_data()
        ycomb = Ycombinator(self.user_name, self.path)
        ycomb.retrieve_data()
        ycomb.lemmatize_data()
        prediction_dataset = techcrunch.techcrunch_df.append(thehackernews.thehackernews_df)
        prediction_dataset = prediction_dataset.append(ycomb.ycombinator_df)
    ...
    def get_daily_english_predictions(self):
        fresh_dataset = self.get_all_data()
        if fresh_dataset is None:
            fresh_dataset = ''
        if len(fresh_dataset) > 0:
            fresh_dataset['prediction'] = self.model.predict_proba(self.transformer.transform(fresh_dataset[
                'lemmatize_title']).toarray())[:,1]
            self.article_history = self.article_history.append(fresh_dataset[['url','title','prediction']]).reset_index(drop=True)
            self.article_history.to_csv(self.path+'english_article_history_'+self.user_name+'.csv', index=False)
